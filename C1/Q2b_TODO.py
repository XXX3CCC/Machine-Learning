...
def train(X_train, y_train, X_val, y_val, decay):
    ...
    for epoch in range(MaxIter):
        ...
        for b in range( int(np.ceil(N_train/batch_size)) ):
            ...
            # theta = theta - (1/M) * alpha * [x * (h(x) - y) + decay * theta]
            X_batch_T = X_batch.transpose()
            w -= (1 / len(y_batch)) * alpha * X_batch_T.dot((y_hat_batch - y_batch)) + alpha * decay * w
...
# Suppose the original features are (x1, x2, ..., xd)^T
# We now extend it as (x1, x2, ..., xd, x1^2, x2^2, ..., xd^2)^T
...
print("1. the best hyperparameter, which is the one yields the best performance: ", decay_best)
print("2. The number of epoch that yields the best validation performance: ", epoch_best)
print("3. The validation performance (risk) in that epoch: ", risk_best)
print("4. The test performance (risk) in that epoch: ", risk_test)
...
plt.tight_layout()
plt.savefig('2(b)_The learning curve of the training loss' + '.jpg')
...
plt.tight_layout()
plt.savefig('2(b)_The learning curve of the validation risk' + '.jpg')
